"""Isolated execution planes for team work."""
...
RUNTIME_LEDGER_PREFIXES = (
    ".contractcoding/runs.sqlite",
    ".contractcoding/events",
)
RUNTIME_LEDGER_SUFFIXES = (
    ".sqlite",
    ".sqlite-journal",
    ".sqlite-wal",
    ".sqlite-shm",
...
class ExecutionPlaneManager:
    ...
    def acquire(
        ...
        requested_mode = (mode_override or self.config.EXECUTION_PLANE or "workspace").strip().lower()
    ...
    def _create_sandbox_plane(
        ...
        shutil.copytree(
            ...
            plane_root,
            ignore=self._copytree_ignore(self.base_workspace_dir),
        )
        return ExecutionPlane(
            mode="sandbox",
    ...
    def _capture_file_hashes(self, root_dir: str) -> Dict[str, Optional[str]]:
        ...
        for current_root, dirs, files in os.walk(root_dir):
            ...
            for file_name in files:
                ...
                if self._should_ignore_rel_path(rel_path):
                    continue
    ...
    def _sync_workspace_snapshot(
        ...
        for rel_path in sorted(baseline_hashes):
            ...
            try:
                ...
                self.logger.debug("Ignoring transient file during workspace snapshot: %s", rel_path)
    ...
    def _copytree_ignore(self, source_root: str):
        def ignore(current_dir: str, names: list[str]) -> set[str]:
            ignored: set[str] = {name for name in names if name in IGNORE_NAMES}
            for name in names:
                rel_path = os.path.relpath(os.path.join(current_dir, name), source_root)
                if self._should_ignore_rel_path(rel_path):
                    ignored.add(name)
            return ignored
    ...
    @staticmethod
    def _should_ignore_rel_path(rel_path: str) -> bool:
        normalized = os.path.normpath(str(rel_path or "").replace("\\", "/")).replace("\\", "/")
        if normalized.startswith("./"):
            normalized = normalized[2:]
        if not normalized:
            return False
        if any(normalized.startswith(prefix) for prefix in RUNTIME_LEDGER_PREFIXES):
            return True
        if normalized.startswith(".contractcoding/") and any(
            normalized.endswith(suffix) for suffix in RUNTIME_LEDGER_SUFFIXES
        ):
            return True
        return False
