...
class GraphTraverser:
    ...
    def traverse(
        ...
        while all_layers[-1] and len(all_layers) <= self.config.MAX_LAYERS:
            ...
            if layer_index == 0:
                next_agent = "Critic"
                task_msg = "Review the Project Plan and Architecture generated by Project Manager. Ensure contracts are clear and complete. If issues found, set status to ERROR in document."
    ...
    def _parse_contract(self, content: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            ...
            if new_file_path:
                if current_task and 'file' in current_task:
    ...
    def _schedule_from_contract(
        ...
        for task in tasks:
            ...
            if status in ['TODO', 'IN_PROGRESS', 'ERROR']:
                ...
                review_msg = (
                    f"Review {file_path}. Current Status: {status}. "
                    "If the implementation is correct and matches the requirements, you MUST use the `document_action` tool "
                    "to update the 'Status' of this file to 'VERIFIED'. "
                    "Do NOT just say it is verified in your text output; you must perform the action. "
                    "If incorrect, set Status to ERROR and delegate fixes."
                )
